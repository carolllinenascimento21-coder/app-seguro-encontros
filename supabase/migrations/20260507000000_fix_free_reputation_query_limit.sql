...
-- This migration intentionally does not reference profiles.plan because some
-- deployed databases only have current_plan_id/subscription fields.
  ...
  if not has_paid_access then
    ...
    update public.profiles
      ...
      returning
        ...
        coalesce(credits, 0)
      into used, current_credits;
  end if;

  insert into public.consultas (user_id) values (user_uuid);

  return query select coalesce(profile_current_plan_id, 'free'), used, current_credits;
end;
