...
export async function GET(request: NextRequest) {
  ...
  if (providerError) {
    console.error('[AUTH RECOVERY COMPLETE] Provider error:', {
      error: providerError,
  ...
  try {
    supabaseEnv = getSupabasePublicEnv('auth/recovery/complete')
  } catch (error) {
    const envError = getMissingSupabaseEnvDetails(error)
    if (envError) {
      console.error('[AUTH RECOVERY COMPLETE] ENV ERROR:', envError.message)
  ...
  if (!supabaseEnv) {
    console.error('[AUTH RECOVERY COMPLETE] Supabase não configurado')
  ...
  if (code) {
    ...
    if (exchangeError) {
      console.error('[AUTH RECOVERY COMPLETE] exchangeCodeForSession falhou:', {
  ...
  if (tokenHash && type === 'recovery') {
    ...
    if (verifyOtpError) {
      console.error('[AUTH RECOVERY COMPLETE] verifyOtp falhou:', {
